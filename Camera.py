...
class Camera:
    ...
    def writeOnStream(self, x, y, width, height, className, frame):
        ...
        if className == 'bee':
            # Draw a Rectangle around detected bee -- orange
            cv2.rectangle(frame, (int(x - width / 2), int(y + height / 2)), (int(x + width / 2), int(y - height / 2)),
                          (0, 165, 255), 2)

            # Draw filled box for bee -- orange
            #cv2.rectangle(frame, (int(x - width / 2), int(y + height / 2)), (int(x + width / 2), int(y + height / 2) + 25),
                      #(0, 165, 255), cv2.FILLED)

            #draw Text
            cv2.putText(frame, className, (int(x - width / 2 + 6), int(y + height / 2 + 26)), font, 1, (255, 255, 255),
                        1)
        else:
            # Draw a Rectangle around detected pollen
            cv2.rectangle(frame, (int(x - width / 2), int(y + height / 2)), (int(x + width / 2), int(y - height / 2)),
                          (255, 255, 0), 2)

            # Draw filled box for pollen
            #cv2.rectangle(frame, (int(x - width / 2), int(y + height / 2)), (int(x + width / 2), int(y + height / 2) + 20),
                          #(255, 255, 0), cv2.FILLED)

            #draw Text
            cv2.putText(frame, className, (int(x - width / 2 + 6), int(y + height / 2 + 3)), font, 0.5, (255, 255, 255),
                        1)

        # cv2.putText(frame, className, (int(x - width / 2 + 6), int(y + height / 2 + 26)), font, 1, (255, 255, 255), 1)
